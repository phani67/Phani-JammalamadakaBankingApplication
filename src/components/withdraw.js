...
export default function Withdraw() {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      if(values.amount <= state.currentUser.balance) {
        ...
        if (amount > state.currentUser.balance) {
          alert("Withdrawal amount cannot be higher than the account balance");
          return;
        }
    ...
    validate: values => {
      ...
      if(values.amount !== "" && isNaN(parseFloat(values.amount))) errors.amount = "Only Numbers are allowed";
  ...
  return (
    ...
    <Card
      ...
      body= {
        ...
        {!state.currentUser && 
          <div>Please login to access this information.</div>
        }
        </>
      }
    />
